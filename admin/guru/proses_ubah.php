...
$mata_pelajaran = mysqli_real_escape_string($koneksi, isset($_POST['mapel']) ? $_POST['mapel'] : '');
...
if($_FILES['foto']['error'] == 0){
	if($_FILES['foto']['size'] < 1000000){
		...
		// ubah data
		$query = mysqli_query($koneksi, "UPDATE tbl_guru SET nama = '$nama', nip = $nip, jenis_kelamin = '$jenis_kelamin', no_hp = '$no_hp', tempat_lahir = '$tempat_lahir', tanggal_lahir = '$tanggal_lahir', id_mapel = '$mata_pelajaran', alamat = '$alamat', foto = '$nama_foto' WHERE id = $id") or die(mysqli_error($koneksi));
		if($query){
