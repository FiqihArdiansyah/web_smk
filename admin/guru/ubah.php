...
$query_mapel = mysqli_query($koneksi, "SELECT id, mapel FROM tbl_mapel");
...
<head>
	...
	<title>Ubah Guru - SMK AL-ITTIHAD RAAS</title>
							...
							<div class="row">
								...
								<select name="mapel" id="mapel" class="form-control">
									<?php while($row = mysqli_fetch_assoc($query_mapel)) : ?>
										<option value="<?= $row['id'] ?>" <?= $guru['id_mapel'] == $row['id'] ? 'selected' : '' ?>><?= $row['mapel'] ?></option>
									<?php endwhile; ?>
								</select>
							</div>
								</div>
